...
_Ostatnia aktualizacja: 2026-08-19 — dodany mnemonik wizualny na żądanie (podwójne kodowanie, Paivio). Wcześniej tego samego dnia: tryb kontekstu/cloze, wymuszona produkcja dla trudnych kart, kontekst krzyżowy na żądanie (Wariant B+D); hierarchia tematów + mastery % (P2-4). Wcześniej (2026-08-07): SCI-1…SCI-7 przeniesione z backlogu (sekcja 4) do „Stan faktyczny" (sekcja 1), zgodnie z rzeczywistym stanem kodu._
...
| **Fiszki: tryb kontekstu (cloze), wymuszona produkcja, kontekst krzyżowy** (Wariant B+D, 2026-08-19) | ✅ Produkcja — (1) tryb cloze: front karty pokazuje `example_sentence` z zasłoniętym słowem zamiast gołej pary słowo→tłumaczenie (RCT: kontekst przy odtwarzaniu bije izolowaną parę — retrieval practice research; zero kosztu AI, dane już istniały na karcie). (2) dla kart w stanie FSRS `Relearning`: pole do wpisania odpowiedzi PRZED odsłonięciem, zamiast czystej samooceny — wymusza realną generację (RCT: efekt generacji, Slamecka & Graf 1978), samoocena 1-4 nadal decyduje o FSRS. (3) „Zobacz w innym kontekście" — na żądanie, AI generuje nowe zdanie dla trudnego słowa w INNYM temacie niż jego własna lekcja (interleaving semantyczny na poziomie pojedynczego słowa, ten sam mechanizm co SCI-4, tylko wywołany ręcznie zamiast automatycznie przy tworzeniu karty). | `frontend/src/pages/Flashcards.jsx` (`buildCloze`, tryb typed-answer, przycisk alt-context), `backend/routers/flashcards.py` (`GET .../due?topic_id=`, `POST .../alt-context`) |
| **Fiszki: mnemonik wizualny na żądanie** (Wariant B, 2026-08-19) | ✅ Produkcja — obraz generowany przez Gemini 2.5 Flash Image (przez OpenRouter lub Gemini Direct, oba już skonfigurowane), TYLKO dla kart z istniejącym tekstowym mnemonikiem (SCI-13) i TYLKO na kliknięcie (real koszt ~$0.02-0.04/obraz — nigdy automatycznie). Podstawa naukowa: podwójne kodowanie (Paivio 1971/1986) — obraz + słowo zostawia dwa niezależne ślady pamięciowe zamiast jednego; ta sama logika co metoda słów kluczowych (Atkinson 1975) już użyta w SCI-13, teraz z realnym obrazem zamiast samego opisu. Wygenerowany raz, zapisany w `Flashcard.mnemonic_image_path` — powtórne żądanie zwraca ten sam plik, nie generuje ponownie. Zweryfikowane na żywo prawdziwym wywołaniem AI na realnej karcie (obraz 1024×1024 PNG poprawnie wygenerowany i wyświetlony), dane testowe wyczyszczone po weryfikacji. | `backend/services/gemini_service.py` (`generate_image`), `backend/services/image_service.py`, `backend/routers/flashcards.py` (`POST .../mnemonic-image`), `frontend/src/pages/Flashcards.jsx` |
